...
class OpenDishwasher(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        if self.camera in [None]:
            ...
            self.camera_place_pos = gymapi.Vec3(0., -1.0, 1.3)
            self.camera_lookat_pos = gymapi.Vec3(0., 0.6, 0.8)
        elif self.camera == 'right':
            self.camera_place_pos = gymapi.Vec3(0.4, -0.9, 1.2)
            self.camera_lookat_pos = gymapi.Vec3(-0.3, 0.6, 0.6)
            ...
            self.camera_place_pos = gymapi.Vec3(1.2, -1.5, 1.5)
        ...
        for i in range(self.num_envs):
            ...
            for object_asset_name in object_assets.keys():
                ...
                if object_asset_name == 'dishwasher':
                    ## joint_0 for door, joint_1 for button 
                    self.door_dof_handle = self.gym.find_actor_dof_handle(env_ptr, object_handle, 'joint_0')
    ...
    def compute_reward(self, actions):
        ...
        if self.repre_type in ["r3m", "ag2manip", "vip", ]:
            ...
            goal_door_dof_pos = 1.0
            door_dof_pos = self.door_dof_pos.clone()
            achived_goal = torch.abs(goal_door_dof_pos - door_dof_pos) < 0.1
            ...
            if self.eureka_seed == 0:
                door_dof_pos = self.door_dof_pos.clone()
                door_dof_vel = self.door_dof_vel.clone()
                target_dof_pos = torch.ones_like(door_dof_pos)
                target_dof_vel = torch.zeros_like(door_dof_vel)
                # we are normalizing the reward to [-1,1] range
                dof_pos_dist_reward = torch.exp(-torch.pow(door_dof_pos - target_dof_pos, 2))
                # we want the gripper to open the door smoothly, so the finishing velocity should be small
                dof_vel_dist_reward = torch.exp(-torch.pow(door_dof_vel - target_dof_vel, 2))
                # the total reward is the sum of all sub rewards
                reward = dof_pos_dist_reward + dof_vel_dist_reward
            elif self.eureka_seed == 1:
                door_dof_pos = self.door_dof_pos.clone()
                door_dof_vel = self.door_dof_vel.clone()
                device = door_dof_pos.device
                dof_goal = torch.tensor([1.0], device=device)
                dof_start = torch.tensor([0.0], device=device)
                # The distance from the door to the goal
                door_to_goal_distance = torch.abs(dof_goal - door_dof_pos)
                # The reward is inversely proportional to the distance to the goal
                distance_reward = torch.exp(-door_to_goal_distance)
                # Encourage the movement of the door
                vel_reward = torch.abs(door_dof_vel)
                # Combine rewards
                # Here we can adjust the coefficients to balance the two parts of the reward
                reward = 0.8*distance_reward + 0.2*vel_reward
            elif self.eureka_seed == 2:
                door_dof_pos = self.door_dof_pos.clone()
                door_dof_vel = self.door_dof_vel.clone()
                # Initialize reward components
                rewards = {}
                goal_pos = torch.tensor(1.0, device=door_dof_pos.device)
                initial_pos = torch.tensor(0.0, device=door_dof_pos.device)
                # Calculate distance to the target
                dist_to_target = torch.abs(goal_pos - door_dof_pos)
                # Reward for moving towards target
                rewards["move_to_target"] = torch.exp(-10.0 * dist_to_target)
                # Reward for reducing velocity when close to the target
                dist_threshold = torch.tensor(0.1, device=door_dof_pos.device)
                near_target = (dist_to_target < dist_threshold).float()
                rewards["reduce_vel"] = -1.0 * door_dof_vel * near_target
                # Combine rewards
        ...
        self.extras['indicator'] = self.door_dof_pos.clone().mean().unsqueeze(0)
    ...
    def compute_observations(self):
        ...
        self.door_dof_pos = self.dof_states[:, self.door_dof_handle, 0]
        self.door_dof_vel = self.dof_states[:, self.door_dof_handle, 1]
