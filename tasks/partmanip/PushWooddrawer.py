...
class PushWooddrawer(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for object_asset_name, object_asset_file in object_asset_files.items():
            ...
            # object_asset_options.vhacd_params.resolution = 10000
        ...
        if self.is_planning or self.act_type in ['robot_joint_pose']:
            ...
            robot_start_pose.p = gymapi.Vec3(0.0, -1.3, 0.8)
            robot_start_pose.r = gymapi.Quat(*R.from_euler('xyz', [0., 0., 1.57]).as_quat())
        else:
            robot_start_pose = gymapi.Transform()
            robot_start_pose.p = gymapi.Vec3(0.0, -1.3, 1.3)
        ...
        for object_asset_name in ["floor", "room_0", "room_1", "wood_drawer"]:
        ...
        if self.camera in [None]:
            ...
            self.camera_place_pos = gymapi.Vec3(0., -1.5, 1.7)
            self.camera_lookat_pos = gymapi.Vec3(0., 0.0, 1.0)
        elif self.camera == 'right':
            self.camera_place_pos = gymapi.Vec3(1.0, -1.2, 1.5)
            self.camera_lookat_pos = gymapi.Vec3(0., 0.0, 1.2)
            ...
            self.camera_place_pos = gymapi.Vec3(1.2, -1.8, 1.8)
            self.camera_lookat_pos = gymapi.Vec3(-0.2, -0.5, 1.0)
        else:
        ...
        for i in range(self.num_envs):
            ...
            for object_asset_name in object_assets.keys():
                ...
                if object_asset_name == 'wood_drawer':
                    self.middle_drawer_dof_handle = self.gym.find_actor_dof_handle(env_ptr, object_handle, 'joint_middle')

                if object_asset_name == 'wood_drawer':
                    dof_states = self.gym.get_actor_dof_states(env_ptr, object_handle, gymapi.STATE_ALL)
                    dof_states['pos'][1] = 0.4
    ...
    def compute_reward(self, actions):
        ...
        if self.repre_type in ["r3m", "ag2manip", "vip", ]:
            ...
            goal_middle_drawer_dof_pos = 0.
            middle_drawer_dof_pos = self.middle_drawer_dof_pos.clone()
            achived_goal = torch.abs(goal_middle_drawer_dof_pos - middle_drawer_dof_pos) < 0.05

            reward = 2. * (0.4 - middle_drawer_dof_pos)
            reward = torch.where(reward < 0, torch.exp(reward) - 1, 10 * (torch.exp(2 * reward) - 1))        
            ...
            if self.eureka_seed == 0:
                middle_drawer_dof_pos = self.middle_drawer_dof_pos.clone()
                goal_drawer_dof = 0.0
                reward = torch.zeros_like(middle_drawer_dof_pos)
                # Define the temperature parameter for reward transformation
                temperature = 10.0
                # Define the reward components
                pos_rew = -torch.abs(middle_drawer_dof_pos - goal_drawer_dof)
                # Apply transformations to the reward components, if any
                pos_rew = torch.exp(temperature * pos_rew)
                reward += pos_rew
            elif self.eureka_seed == 1:
                middle_drawer_dof_pos = self.middle_drawer_dof_pos.clone()
                goal_pos = 0.0
                # initialize reward components
                completion_reward: torch.Tensor = torch.zeros_like(middle_drawer_dof_pos)
                movement_reward: torch.Tensor = torch.zeros_like(middle_drawer_dof_pos)
                # fine tuning parameters
                movement_reward_scale: float = 0.1
                completion_reward_scale: float = 1.0
                # calculate the distance to the goal
                dist_to_goal: torch.Tensor = torch.abs(middle_drawer_dof_pos - goal_pos)
                movement_reward = -movement_reward_scale * dist_to_goal
                # provide a completion reward when the gripper reaches the middle drawer
                is_complete: torch.Tensor = (dist_to_goal < 0.01).float()
                completion_reward = is_complete * completion_reward_scale
                # calculate the total reward
                reward: torch.Tensor = movement_reward + completion_reward
            elif self.eureka_seed == 2:
                middle_drawer_dof_pos = self.middle_drawer_dof_pos.clone()
                middle_drawer_dof_vel = self.middle_drawer_dof_vel.clone()
                # Define temperature variable
                temp = torch.tensor([0.01]).to(middle_drawer_dof_pos.device)
                # Define target position of the middle drawer
                target_drawer_pos = torch.tensor([0.0]).to(middle_drawer_dof_pos.device)
                # Calculate the distance between current position and target position and its velocity
                pos_dist = torch.abs(middle_drawer_dof_pos - target_drawer_pos)
                vel_penalty = middle_drawer_dof_vel.norm()
                # Apply transformation to the position difference and velocity with the temperature variable
                pos_reward = torch.exp(-temp * pos_dist)
                vel_reward = torch.exp(-temp * vel_penalty)
        ...
        self.scores = 1.0 - (middle_drawer_dof_pos) / 0.4
        ...
        self.extras['indicator'] = self.middle_drawer_dof_pos.clone().mean().unsqueeze(0)
    ...
    def compute_observations(self):
        ...
        self.middle_drawer_dof_pos = self.dof_states[:, self.middle_drawer_dof_handle, 0]
        self.middle_drawer_dof_vel = self.dof_states[:, self.middle_drawer_dof_handle, 1]
    ...
    def _setup_attachable_body(self, ):
        ...
        for object_name in ['wood_drawer']:
            ## load attach info
            if object_name == 'wood_drawer':
                object_attachs = pickle.load(open(os.path.join(env_asset_root, 'attachable', f'attach_{object_name}_middle0.5.pkl'), 'rb'))['data']
            else:
