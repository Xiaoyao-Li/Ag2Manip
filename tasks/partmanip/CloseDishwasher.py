...
class CloseDishwasher(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        if self.is_planning or self.act_type in ['robot_joint_pose']:
            ...
            robot_start_pose.p = gymapi.Vec3(0.0, -1.0, 0.4)
            robot_start_pose.r = gymapi.Quat(*R.from_euler('xyz', [0., 0., 1.57]).as_quat())
        else:
            robot_start_pose = gymapi.Transform()
            robot_start_pose.p = gymapi.Vec3(0.0, -1.0, 1.0)
    ...
    def compute_reward(self, actions):
        ...
        if self.repre_type in ["r3m", "ag2manip", "vip", ]:
            ...
            if self.eureka_seed == 0:
                ...
                # The goal position of the door is 0.0
                goal_pos = torch.zeros_like(door_dof_pos)
                # The reward based on the distance between door position and the goal
                distance_reward = torch.exp(-torch.linalg.norm(door_dof_pos - goal_pos))
                # The velocity of door should be low, reward this
                grip_speed_reward = 1.0 - torch.exp(-torch.linalg.norm(door_dof_vel))
                # The total reward combining the distance and grip speed
                reward = distance_reward + grip_speed_reward
            elif self.eureka_seed == 1:
                door_dof_pos = self.door_dof_pos.clone()
                goal_pos = 0.0
                # reward is high when the door dof position is close to 0
                reward_dist_to_target = 1.0 - torch.abs(door_dof_pos - goal_pos)
                # The total reward is the sum of the above reward components
                reward = reward_dist_to_target
            elif self.eureka_seed == 2:
                door_dof_pos = self.door_dof_pos.clone()
                # define the goal position of the dishwasher door
                goal_pos = torch.tensor(0.0, device=door_dof_pos.device)
                # compute the distance to the goal
                distance = torch.abs(goal_pos - door_dof_pos)
                # define temperature for the exponential function
                temperature = torch.tensor(0.1, device=door_dof_pos.device)
                # apply the exponential transformation to the distance
                distance_reward = torch.exp(-temperature * distance)
