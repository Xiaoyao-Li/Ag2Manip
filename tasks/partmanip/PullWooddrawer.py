...
class PullWooddrawer(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        if self.is_planning or self.act_type in ['robot_joint_pose']:
            ...
            robot_start_pose.p = gymapi.Vec3(0.0, -1.3, 0.7)
    ...
    def compute_reward(self, actions):
        ...
        if self.repre_type in ["r3m", "ag2manip", "vip", ]:
            ...
            goal_middle_drawer_dof_pos = 0.4
            ...
            reward = middle_drawer_dof_pos / 0.4
            reward = torch.where(reward < 0, torch.exp(reward) - 1, 10 * (torch.exp(2 * reward) - 1))
        
        elif self.repre_type in ["eureka"]:
            if self.eureka_seed == 0:
                target_pos = 0.4
                dof_pos = self.middle_drawer_dof_pos.clone()
                target = torch.Tensor([target_pos])
                reward_dict = {}
                # Safeguard to ensure tensors are on the same device
                if dof_pos.device != target.device:
                    target = target.to(dof_pos.device)
                # Reward for being close to the target position
                pos_reward = torch.exp(-1.0 * torch.abs(dof_pos - target))
                reward_dict["pos_reward"] = pos_reward
                # Total reward is just positional reward
                reward = pos_reward
            elif self.eureka_seed == 1:
                middle_drawer_dof_pos = self.middle_drawer_dof_pos.clone()
                goal_pos = 0.4
                # define the temperature parameter for the transformed reward component
                temp_parameter = 0.1
                # compute the distance to the goal
                distance_to_goal = torch.abs(middle_drawer_dof_pos - goal_pos)
                # apply an exponential transformation to normalize the distance
                reward_distance = torch.exp(-temp_parameter * distance_to_goal)
                # reward for opening the drawer
                reward_open = (middle_drawer_dof_pos > 0).float()
                # compute the total reward
                reward = reward_distance + reward_open
            elif self.eureka_seed == 2:
                middle_drawer_dof_pos = self.middle_drawer_dof_pos.clone()
                goal_pos = 0.4
                # Defining the temperature
                temp = torch.tensor(1.0).to(middle_drawer_dof_pos.device)
                # Define reward for being close to goal
                reward_goal = torch.exp(-1.0 * temp * torch.abs(middle_drawer_dof_pos - goal_pos))
                # the total reward is just reward_goal for now
                reward = reward_goal
            else:
                raise NotImplementedError
        ...
        self.scores = middle_drawer_dof_pos / 0.4
