...
class PressButton(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        if self.is_planning or self.act_type in ['robot_joint_pose']:
            ...
            robot_start_pose.p = gymapi.Vec3(0.0, -1.0, 0.7)
            robot_start_pose.r = gymapi.Quat(*R.from_euler('xyz', [0., 0., 1.57]).as_quat())
        else:
            robot_start_pose = gymapi.Transform()
            robot_start_pose.p = gymapi.Vec3(0.0, -0.8, 1.2)
        ...
        if self.camera in [None]:
            ...
            self.camera_place_pos = gymapi.Vec3(0., -0.8, 1.2)
            self.camera_lookat_pos = gymapi.Vec3(0., 0.6, 0.6)
        elif self.camera == 'right':
            self.camera_place_pos = gymapi.Vec3(0.4, -0.8, 1.2)
        ...
        for i in range(self.num_envs):
            ...
            for object_asset_name in object_assets.keys():
                ...
                if object_asset_name == 'dishwasher':
                    ...
                    self.button_dof_handle = self.gym.find_actor_dof_handle(env_ptr, object_handle, 'joint_1')

                if object_asset_name == 'dishwasher':
                    dof_states = self.gym.get_actor_dof_states(env_ptr, object_handle, gymapi.STATE_ALL)
                    dof_states['pos'][1] = -0.01
    ...
    def compute_reward(self, actions):
        ...
        if self.repre_type in ["r3m", "ag2manip", "vip", ]:
            ...
            goal_button_dof_pos = 0.01
            button_dof_pos = self.button_dof_pos.clone()
            achived_goal = torch.abs(goal_button_dof_pos - button_dof_pos) < 0.01

            reward = 50 * (0.01 + button_dof_pos)
            reward = torch.where(reward < 0, torch.exp(reward) - 1, 10 * (torch.exp(2 * reward) - 1))
        
        elif self.repre_type in ["eureka"]:
            if self.eureka_seed == 0:
                button_dof_pos = self.button_dof_pos.clone()
                button_dof_vel = self.button_dof_vel.clone()
                # The goal position for the button
                goal_pos = torch.tensor([0.01], device=button_dof_pos.device)
                # The reward is the negative distance from the current position to the goal position
                reward_dist = -torch.abs(button_dof_pos - goal_pos)
                # Adding penalty for the button velocity
                reward_vel = -torch.abs(button_dof_vel)
                # Use a temperature parameter to balance the importance of the distance and velocity rewards
                temperature = torch.tensor([0.5], device=button_dof_pos.device)
                reward = temperature * reward_dist + (1 - temperature) * reward_vel
                ...
                temp_pos: float = 0.01
                temp_vel: float = 0.02
                # calculate the positional error (distance from goal)
                pos_error = button_dof_pos - goal_pos
                positional_reward = torch.exp(-temp_pos * torch.abs(pos_error)) * pos_weight
                # calculate the velocity error (we want the button to stay pressed)
                velocity_reward = torch.exp(-temp_vel * torch.abs(button_dof_vel)) * vel_weight
                # combine the rewards into a total reward
                reward = positional_reward + velocity_reward
            elif self.eureka_seed == 2:
                button_dof_pos = self.button_dof_pos.clone()
                button_dof_vel = self.button_dof_vel.clone()
                button_press_reward_velocity = 0.01 - torch.abs(button_dof_vel)
                button_press_reward_position = torch.abs(button_dof_pos - 0.01)
                reward = button_press_reward_position + button_press_reward_velocity
            else:
                raise NotImplementedError
        ...
        self.scores = 50 * (0.01 + button_dof_pos)
        ...
        self.extras['indicator'] = self.button_dof_pos.clone().mean().unsqueeze(0)
    ...
    def compute_observations(self):
        ...
        self.button_dof_pos = self.dof_states[:, self.button_dof_handle, 0]
        self.button_dof_vel = self.dof_states[:, self.button_dof_handle, 1]
