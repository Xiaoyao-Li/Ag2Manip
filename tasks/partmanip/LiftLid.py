...
class LiftLid(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        if self.is_planning or self.act_type in ['robot_joint_pose']:
            ...
            robot_start_pose.p = gymapi.Vec3(0.0, -0.7, 0.6)
            robot_start_pose.r = gymapi.Quat(*R.from_euler('xyz', [0., 0., 1.57]).as_quat())
        else:
            robot_start_pose = gymapi.Transform()
            robot_start_pose.p = gymapi.Vec3(0.0, -0.7, 1.2)
        ...
        for object_asset_name in ["floor", "room_0", "room_1", "pot", "basecube"]:
        ...
        if self.camera in [None]:
            ...
            self.camera_place_pos = gymapi.Vec3(0., -0.7, 1.2)
            self.camera_lookat_pos = gymapi.Vec3(0., 0.7, 0.6)
        elif self.camera == 'right':
            self.camera_place_pos = gymapi.Vec3(0.4, -0.7, 1.2)
            self.camera_lookat_pos = gymapi.Vec3(-0.3, 0.7, 0.6)
            ...
            self.camera_place_pos = gymapi.Vec3(-1.5, -1.3, 1.8)
            self.camera_lookat_pos = gymapi.Vec3(0.5, -0.5, 1.0)
        else:
        ...
        for i in range(self.num_envs):
            ...
            for object_asset_name in object_assets.keys():
                ...
                if object_asset_name == 'pot':
                    ...
                    self.lid_dof_handle = self.gym.find_actor_dof_handle(env_ptr, object_handle, 'joint_0')
    ...
    def compute_reward(self, actions):
        ...
        if self.repre_type in ["r3m", "ag2manip", "vip", ]:
            ...
            goal_lid_dof_pos = 0.2
            lid_dof_pos = self.lid_dof_pos.clone()
            achived_goal = torch.abs(goal_lid_dof_pos - lid_dof_pos) < 0.03

            reward = lid_dof_pos / 0.2
            reward = torch.where(reward < 0, torch.exp(reward) - 1, 10 * (torch.exp(2 * reward) - 1))
        
        elif self.repre_type in ["eureka"]:
            if self.eureka_seed == 0:
                lid_dof_pos = self.lid_dof_pos.clone()
                lid_dof_vel = self.lid_dof_vel.clone()
                goal_pos = torch.tensor(0.2, device=lid_dof_pos.device)
                initial_pos = torch.tensor(0.0, device=lid_dof_pos.device)
                lid_position_reward = -torch.abs(lid_dof_pos - goal_pos)
                lid_velocity_reward = -torch.abs(lid_dof_vel)
                reward = lid_position_reward + lid_velocity_reward
            elif self.eureka_seed == 1:
                lid_dof_pos = self.lid_dof_pos.clone()
                lid_dof_vel = self.lid_dof_vel.clone()
                target_pos = torch.tensor(0.2)        
                reward_dict = {}
                # Reward for moving the lid up
                reward_pos = target_pos - lid_dof_pos
                reward_vel = torch.exp(-lid_dof_vel.abs())
                reward = reward_pos + reward_vel
            elif self.eureka_seed == 2:
                lid_dof_pos = self.lid_dof_pos.clone()
                goal_pos = torch.tensor(0.2, device=lid_dof_pos.device)
                # Determine the distance error by subtracting the goal position from the current lid position
                distance_error = goal_pos - lid_dof_pos
                # Create a temperature variable for applying it inside the transformation function.
                distance_temperature = torch.tensor(0.1, device=distance_error.device)
                # Use exponential function to normalize the distance reward to a fixed range
                distance_reward = torch.exp(-distance_temperature * torch.abs(distance_error))
                # The total reward is just the distance reward 
        ...
        self.extras['indicator'] = self.lid_dof_pos.clone().mean().unsqueeze(0)
    ...
    def compute_observations(self):
        ...
        self.lid_dof_pos = self.dof_states[:, self.lid_dof_handle, 0]
        self.lid_dof_vel = self.dof_states[:, self.lid_dof_handle, 1]
