...
class OpenSlidecabinet(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        if self.is_planning or self.act_type in ['robot_joint_pose']:
            ...
            robot_start_pose.p = gymapi.Vec3(0.0, -0.6, 2.0)  ## default pose
    ...
    def compute_reward(self, actions):
        ...
        if self.repre_type in ["r3m", "ag2manip", "vip", ]:
            ...
            if self.eureka_seed == 0:
                ...
                slidedoor_dof_pos = self.slidedoor_dof_pos.clone()
                distance_reward_raw = torch.abs(target_pos - slidedoor_dof_pos)
                distance_reward = torch.exp(-reward_temp1 * distance_reward_raw)
                completion_reward = torch.zeros(self.num_envs, device=slidedoor_dof_pos.device)
                completion_reward = torch.where(distance_reward_raw < target_tol, torch.ones_like(completion_reward), completion_reward)
                # if (target_pos - slidedoor_dof_pos).abs() < target_tol:
                #     completion_reward_raw = torch.tensor(1.0, device=slidedoor_dof_pos.device)
                completion_reward = torch.exp(reward_temp2 * completion_reward)
                # sum up the reward components to calculate total reward
                reward = distance_reward + completion_reward
            elif self.eureka_seed == 1:
                # control costs
                robot_dof_vel = self.robot_dof_vel.clone()
                control_scale = torch.tensor(1.0, device=robot_dof_vel.device)
                control_cost = torch.mean(torch.square(robot_dof_vel)) * control_scale
                # goal proximity reward
                slidedoor_dof_pos = self.slidedoor_dof_pos.clone()
                door_open_value = torch.tensor(0.44, device=slidedoor_dof_pos.device)
                door_open_proximity_reward = torch.exp(-torch.square(door_open_value - slidedoor_dof_pos))
                # total reward
                reward = door_open_proximity_reward - control_cost
            elif self.eureka_seed == 2:
                goal_pos = 0.44
                slide_temp = 2.0 # Temperature for slide door position reward
                goal_temp = 2.0  # Temperature for goal position reward
                slidedoor_pos = self.slidedoor_dof_pos.clone()
                target_reached = torch.abs(slidedoor_pos - goal_pos)
                # Scaling reward component to make it between -1 and 1
                slide_reward = torch.exp(-slide_temp * torch.norm(slidedoor_pos))
                goal_reward = torch.exp(-goal_temp * torch.norm(target_reached))
                reward = slide_reward + goal_reward
            else:
                raise NotImplementedError
        ...
        self.scores = slidedoor_dof_pos.clone() / 0.44
        self.scores = torch.where(self.scores > 1., torch.ones_like(self.scores), self.scores)
    ...
    def compute_observations(self):
        ...
        ## for logging the training success score curve
