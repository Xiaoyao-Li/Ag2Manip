...
class TurnoffSwitch(BaseTask):
    ...
    def compute_reward(self, actions):
        ...
        if self.repre_type in ["r3m", "ag2manip", "vip", ]:
            ...
            goal_switch_dof_pos = 0.0
            switch_dof_pos = self.switch_dof_pos.clone()
            achived_goal = torch.abs(switch_dof_pos - goal_switch_dof_pos) < 0.2

            reward = 2 * (0.7 - torch.abs(switch_dof_pos - goal_switch_dof_pos))
        
        elif self.repre_type in ['eureka']:
            if self.eureka_seed == 0:
                goal_pos = 0.0
                switch_dof_pos = self.switch_dof_pos.clone()
                goal_reached = torch.abs(switch_dof_pos - goal_pos) < 0.01
                goal_reward = torch.where(goal_reached, torch.tensor(1.0, device=switch_dof_pos.device), torch.tensor(-1.0, device=switch_dof_pos.device))
                progress_reward = -torch.abs(switch_dof_pos - goal_pos)
                bonus = torch.zeros(self.num_envs, device=switch_dof_pos.device)
                bonus = torch.where(goal_reached, 1.0, bonus)
                reward = goal_reward + progress_reward + bonus   #* summing up the rewards
            elif self.eureka_seed == 1:
                goal_pos = 0.0
                switch_dof_pos = self.switch_dof_pos.clone()
                temp = torch.tensor(0.2)  # temperature variable
                dist_to_goal = torch.sub(goal_pos, switch_dof_pos)
                norm_dist = torch.div(dist_to_goal, temp)
                reward_dist = torch.exp(-norm_dist)
                # If the distance to goal is less than a small threshold, the agent successfully turns off the switch.
                success_reward = (dist_to_goal < 0.01).float()
                reward = reward_dist + success_reward
            elif self.eureka_seed == 2:
                switch_dof_pos = self.switch_dof_pos.clone()
                switch_dof_vel = self.switch_dof_vel.clone()
                switch_dof_pos_heat = 10.0  # Heat parameter for switch_dof_pos, can tune this
                # First reward is based on progressing towards goal position
                progress_reward = torch.clamp(-0.7 - switch_dof_pos, min=0.0)
                progress_reward_heat = 5.0  # Heat parameter for progress_reward, can tune this
                progress_reward = torch.exp(-progress_reward_heat * progress_reward)
                # Penalty for non-zero velocity
                vel_penalty = torch.exp(-switch_dof_pos_heat * torch.abs(switch_dof_vel))
                vel_penalty_heat = 30.0  # Heat parameter for velocity_penalty, can tune this
                # Combine the rewards and penalties
                reward = progress_reward - vel_penalty
            else:
                raise NotImplementedError
        else:
            raise NotImplementedError
        ...
        self.scores = (0.7 + switch_dof_pos.clone()) / 0.7
    ...
    def _setup_attachable_body(self, ):
        ...
        for object_name in ['kettle', 'hingecabinet', 'slidecabinet', 'microwave', 'oven']:
            ## load attach info
            if object_name == 'oven':
                object_attachs = pickle.load(open(os.path.join(env_asset_root, 'attachable', f'attach_{object_name}_switch-0.7.pkl'), 'rb'))['data']
            else:
