...
class PickupKettle(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        # actuated_dof_names = [self.gym.get_asset_actuator_joint_name(robot_asset, i) for i in range(self.num_robot_actuators)]
        ...
        # self.robot_dof_indices = [self.gym.find_asset_dof_index(robot_asset, name) for name in robot_dof_names]
        ...
        if self.is_planning or self.act_type in ['robot_joint_pose']:
            ...
            robot_start_pose.p = gymapi.Vec3(0.0, -0.9, 1.4)
            robot_start_pose.r = gymapi.Quat(*R.from_euler('xyz', [0., 0., 1.57]).as_quat())
        else:
            robot_start_pose = gymapi.Transform()
            robot_start_pose.p = gymapi.Vec3(0.0, -0.9, 2.0)
        ...
        if self.camera in [None]:
            ...
            self.camera_place_pos = gymapi.Vec3(-0.2, -1.1, 2.1)
            self.camera_lookat_pos = gymapi.Vec3(-0.2, 0.0, 2.0)
        elif self.camera == 'right':
            self.camera_place_pos = gymapi.Vec3(0.1, -1.0, 2.1)
            self.camera_lookat_pos = gymapi.Vec3(-0.2, -0.5, 2.0)
        elif self.camera == 'left':
            self.camera_place_pos = gymapi.Vec3(-0.5, -1.0, 2.1)
            self.camera_lookat_pos = gymapi.Vec3(-0.2, -0.5, 2.0)
        elif self.camera == 'demo':
            self.camera_props.width = 3840
            self.camera_props.height = 2160
            self.camera_place_pos = gymapi.Vec3(1.2, -1.0, 2.5)
            self.camera_lookat_pos = gymapi.Vec3(-0.2, -1.0, 1.8)
        else:
        ...
        for i in range(self.num_envs):
            ...
            for object_asset_name in object_assets.keys():
                ...
                if object_asset_name == "kettle":
                    ...
                    self.kettle_body_handle = self.gym.get_actor_rigid_body_handle(env_ptr, object_handle, 1)
            if i == 0:
        ...
        print(f'kettle body handle: {self.kettle_body_handle}')
    ...
    def compute_reward(self, actions):
        ...
        if self.repre_type in ["r3m", "ag2manip", "vip", ]:
            ...
            goal_kettle_pose = torch.tensor([-0.169, -0.5, 1.9, 0., 0., 0., 1.], device=self.device)
            kettle_pose = self.kettle_pose.clone()
            achived_goal = torch.norm(kettle_pose - goal_kettle_pose, dim=-1) < 0.1

            reward = 1 - torch.norm(kettle_pose[:, :3] - goal_kettle_pose[:3], dim=1) / 0.27
            reward = torch.where(reward < 0, torch.exp(reward) - 1, 10 * (torch.exp(2 * reward) - 1))
        
        elif self.repre_type in ['eureka']:
            if self.eureka_seed == 0:
                kettle_pose = self.kettle_pose.clone()[:, :3]
                goal_pos = torch.tensor([-0.169, -0.5, 1.9], device=kettle_pose.device)
                # calculating the distance between the object's current position and the goal position
                distance_to_goal = torch.dist(goal_pos, kettle_pose)
                # defining the temperature parameter for the reward transformation
                temperature = torch.tensor(0.1, device=kettle_pose.device)
                # transforming the reward by taking the negative exponential of the distance
                reward = -torch.exp(distance_to_goal / temperature)
            elif self.eureka_seed == 1:
                kettle_pose = self.kettle_pose.clone()[:, :3]
                goal_pos = torch.tensor([-0.169, -0.5, 1.9], device=kettle_pose.device)
                # Define the reward temperature
                reward_temp = torch.tensor(1.0, device=kettle_pose.device)
                # Compute the distance between the current and goal position
                distance = torch.sqrt(torch.sum((goal_pos - kettle_pose)**2, dim=-1))
                # Compute reward as the negative distance, which encourages the agent to minimize the distance
                reward = -distance / reward_temp
            elif self.eureka_seed == 2:
                kettle_pose = self.kettle_pose.clone()[:, :3]
                goal_pos = torch.tensor([-0.169, -0.5, 1.9], device=kettle_pose.device)
                kettle_pos_current = kettle_pose[:, 0:3]
                kettle_pos_z_current = kettle_pose[:, 2]
                kettle_pos_z_goal = goal_pos[2]
                # calculate the Euclidean distance
                dist_current = torch.sqrt(torch.sum((kettle_pos_current - goal_pos)**2, dim=-1))
                # calculate the height difference
                height_diff = kettle_pos_z_goal - kettle_pos_z_current
                # give more reward to vertical movement towards the goal
                reward_height = torch.exp(-20*height_diff)
                # normalize the distance to a range (0,1)
                dist_current_norm = torch.exp(-0.2*dist_current)
                reward = 0.8*dist_current_norm + 0.2*reward_height
            else:
                raise NotImplementedError
        else:
            raise NotImplementedError
        ...
        self.scores = 1 - torch.norm(kettle_pose[:, :3] - goal_kettle_pose[:3], dim=1) / 0.27
        ...
        self.extras['indicator'] = self.kettle_pose.clone().mean(dim=0)
    ...
    def compute_observations(self):
        ...
        self.kettle_pose = self.rigid_body_states[:, self.kettle_body_handle, 0:7]
        self.kettle_linvel = self.rigid_body_states[:, self.kettle_body_handle, 7:10]
        self.kettle_angvel = self.rigid_body_states[:, self.kettle_body_handle, 10:13]
