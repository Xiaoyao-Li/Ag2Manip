...
class MoveKettle(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        if self.camera in [None]:
            ...
            self.camera_place_pos = gymapi.Vec3(0.0, -0.9, 2.5)
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.0, 1.8)
        elif self.camera == 'right':
            self.camera_place_pos = gymapi.Vec3(0.3, -0.8, 2.4)
            self.camera_lookat_pos = gymapi.Vec3(0.0, -0.2, 1.8)
            ...
            self.camera_place_pos = gymapi.Vec3(0., -2.3, 3.0)
            self.camera_lookat_pos = gymapi.Vec3(0., 0.5, 1.3)
        else:
    ...
    def compute_reward(self, actions):
        ...
        if self.repre_type in ["r3m", "ag2manip", "vip", ]:
            ...
            goal_kettle_pose = torch.tensor([-0.169, -0.2, 1.6219, 0., 0., 0., 1.], device=self.device)
            kettle_pose = self.kettle_pose.clone()
            
            achived_goal = torch.norm(kettle_pose - goal_kettle_pose, dim=-1) < 0.1

            reward = 1 - torch.norm(kettle_pose[:, :3] - goal_kettle_pose[:3], dim=1) / 0.3
            reward = torch.where(reward < 0, torch.exp(reward) - 1, 10 * (torch.exp(2 * reward) - 1))
        
        elif self.repre_type in ['eureka']:
            if self.eureka_seed == 0:
                # Ensure all tensors are on the same device as input tensors
                kettle_pos = self.kettle_pose.clone()[:, :3]
                goal_pos = torch.tensor([-0.169, -0.2, 1.6219], device=kettle_pos.device)
                goal_pos = goal_pos.to(kettle_pos.device)
                # Parameters for rewarding the agent
                reach_goal_reward_weight = torch.tensor(1.0, device=kettle_pos.device)
                stay_close_reward_weight = torch.tensor(0.1, device=kettle_pos.device)
                # Distance to the goal
                distance_to_goal = torch.norm(kettle_pos - goal_pos, dim=-1)
                # Reward for reaching the goal
                reach_goal_reward = (distance_to_goal < 0.02).float() * reach_goal_reward_weight
                # Reward for staying close to the goal
                stay_close_reward = (1.0 - torch.tanh(10.0 * distance_to_goal)) * stay_close_reward_weight
                rewards = {'reach_goal_reward': reach_goal_reward, 
                        'stay_close_reward': stay_close_reward}
                # total reward is the sum of all individual reward components
                reward = torch.stack(list(rewards.values())).sum(dim=0)
            elif self.eureka_seed == 1:
                kettle_pose = self.kettle_pose.clone()[:, :3]
                goal_pos = torch.tensor([-0.169, -0.2, 1.6219], device=kettle_pose.device)
                # device detection
                device = kettle_pose.device
                # constants for reward calculation
                goal_tolerance = torch.tensor(0.005, device=device)
                temp_distance = torch.tensor(0.1, device=device)
                # calculate distance to goal
                distance = torch.sqrt(torch.sum((kettle_pose-goal_pos)**2))
                # compute reward
                if distance < goal_tolerance:
                    reward = torch.tensor(1., device=device) 
                else:
                    # apply transformation to reward such that it's normalized and decreases with distance
                    reward = torch.exp(-temp_distance * distance)
            elif self.eureka_seed == 2:
                kettle_pose = self.kettle_pose.clone()[:, :3]
                goal_pos = torch.tensor([-0.169, -0.2, 1.6219], device=kettle_pose.device)
                distance_to_goal = torch.norm(kettle_pose - goal_pos, dim=-1)
                progress = torch.clamp(1.0 - distance_to_goal, min=0.0)
                reward_push = progress
                reward_dist = -distance_to_goal
                reward = reward_push + reward_dist
            else:
                raise NotImplementedError
        else:
            raise NotImplementedError
        ...
        self.scores = 1 - torch.norm(kettle_pose[:, 1:2] - goal_kettle_pose[1:2], dim=1) / 0.3
