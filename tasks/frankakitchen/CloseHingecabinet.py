...
class CloseHingecabinet(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        if self.is_planning or self.act_type in ['robot_joint_pose']:
            ...
            robot_start_pose.p = gymapi.Vec3(-0.1, -1., 2.0)
            robot_start_pose.r = gymapi.Quat(*R.from_euler('xyz', [0., 0., 3.14]).as_quat())
        else:
            robot_start_pose = gymapi.Transform()
            robot_start_pose.p = gymapi.Vec3(-0.1, -1., 2.6)
    ...
    def compute_reward(self, actions):
        ...
        if self.repre_type in ["r3m", "ag2manip", "vip", ]:
            ...
            goal_hingedoorleft_dof_pos = 0.0
            hingedoorleft_dof_pos = self.hingedoorleft_dof_pos.clone()
            
            achived_goal = torch.abs(hingedoorleft_dof_pos - goal_hingedoorleft_dof_pos) < 0.1
            reward = 2 * (1.0 - torch.abs(hingedoorleft_dof_pos - goal_hingedoorleft_dof_pos))
            reward = torch.where(reward < 0, torch.exp(reward) - 1, 10 * (torch.exp(2 * reward) - 1))
            ...
            if self.eureka_seed == 0:
                door_goal = 0.0
                hingedoorleft_dof_pos = self.hingedoorleft_dof_pos.clone()
                door_diff = hingedoorleft_dof_pos - door_goal
                door_reward = 1.0 - torch.tanh(torch.abs(door_diff))
                # Check if the door is closed, and if so, provide a negative reward
                door_closed = torch.where(hingedoorleft_dof_pos <= -1.0, torch.ones_like(hingedoorleft_dof_pos), torch.zeros_like(hingedoorleft_dof_pos))
                door_closed_reward = -door_closed
                temp_door = 10.0
                transformed_door_reward = torch.exp(temp_door * door_reward)  
                reward = transformed_door_reward + door_closed_reward
            elif self.eureka_seed == 1:
                reward_dist_goal = 1.0
                reward_velocity = 0.02
                hingedoorleft_dof_pos = self.hingedoorleft_dof_pos.clone()
                dof_goal = torch.tensor(0.0, device=hingedoorleft_dof_pos.device)
                # calculate the distance from the current position to the goal
                dist =  torch.abs(dof_goal - hingedoorleft_dof_pos)
                # calculate the final reward for the distance to the goal, closer to goal gets higher reward
                reward_dist = torch.exp(-1.0 * reward_dist_goal * dist)
                # calculate reward for the negative velocity, faster openning speed gets higher reward
                # since the door opens from -1 to 0, the dof velocity is negative.
                hingedoorleft_dof_vel = self.hingedoorleft_dof_vel.clone()
                reward_vel = torch.clamp(-1.0 * hingedoorleft_dof_vel, min=0.0)
                # calculate the final reward
                reward = reward_dist + reward_velocity * reward_vel
            elif self.eureka_seed == 2:
                # Define reward weights
                door_pos = self.hingedoorleft_dof_pos.clone()
                door_vel = self.hingedoorleft_dof_vel.clone()
                w_pos = torch.tensor(1.0, device=door_pos.device)
                w_speed = torch.tensor(0.1, device=door_pos.device)
                # Define goal values
                goal_pos = torch.tensor(0.0, device=door_pos.device)
                goal_speed = torch.tensor(0.0, device=door_pos.device)
                # Define reward components
                r_dist = w_pos * torch.exp(-torch.abs(goal_pos - door_pos))              # reward based on proximity to goal position
                r_speed = w_speed * torch.exp(-torch.abs(goal_speed - door_vel))         # reward based on speed to be maintained
                reward = r_dist + r_speed
            else:
                raise NotImplementedError
        else:
            raise NotImplementedError
        ...
        self.scores = torch.relu(1.0 + hingedoorleft_dof_pos)
    ...
    def _setup_attachable_body(self, ):
        ...
        for object_name in ['kettle', 'hingecabinet', 'slidecabinet', 'microwave', 'oven']:
            ## load attach info
            if object_name == 'hingecabinet':
                object_attachs = pickle.load(open(os.path.join(env_asset_root, 'attachable', f'attach_{object_name}_door-1.0.pkl'), 'rb'))['data']
            else:
