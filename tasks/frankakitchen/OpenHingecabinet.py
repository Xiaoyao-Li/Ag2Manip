...
class OpenHingecabinet(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        if self.is_planning or self.act_type in ['robot_joint_pose']:
            ...
            robot_start_pose.p = gymapi.Vec3(-0.2, -1., 2.0)
            robot_start_pose.r = gymapi.Quat(*R.from_euler('xyz', [0., 0., 3.14]).as_quat())
        else:
            robot_start_pose = gymapi.Transform()
            robot_start_pose.p = gymapi.Vec3(-0.2, -1., 2.6)
        ...
        if self.camera in [None]:
            ...
            self.camera_place_pos = gymapi.Vec3(0.3, -1.1, 2.9)
            self.camera_lookat_pos = gymapi.Vec3(-1.0, -1.1, 2.0)
        elif self.camera == 'right':
            self.camera_place_pos = gymapi.Vec3(0.1, -0.7, 2.8)
            self.camera_lookat_pos = gymapi.Vec3(-1.0, -1.1, 2.1)
        elif self.camera == 'left':
            self.camera_place_pos = gymapi.Vec3(0.1, -1.5, 2.8)
            self.camera_lookat_pos = gymapi.Vec3(-1.0, -1.1, 2.1)
        elif self.camera == 'demo':
            self.camera_props.width = 3840
            self.camera_props.height = 2160
            self.camera_place_pos = gymapi.Vec3(0.8, -2.5, 3.5)
            self.camera_lookat_pos = gymapi.Vec3(-1.0, 0.5, 1.8)
        else:
        ...
        for i in range(self.num_envs):
            ...
            for object_asset_name in object_assets.keys():
                ...
                if object_asset_name == "hingecabinet":
                ...
                if object_asset_name == "hingecabinet":
                    ## dof names: [leftdoorhinge, rightdoorhinge]
                    ## rigid body names: ['', 'hingecab', 'hingeleftdoor', 'hingerightdoor']
                    self.hingedoorleft_dof_handle = self.gym.get_actor_dof_handle(env_ptr, object_handle, 0)
                    self.hingedoorleft_body_handle = self.gym.get_actor_rigid_body_handle(env_ptr, object_handle, 2)
            if i == 0:
        ...
        print(f'hingedoorleft dof handle: {self.hingedoorleft_dof_handle}')
        print(f'hingedoorleft body handle: {self.hingedoorleft_body_handle}')
    ...
    def compute_reward(self, actions):
        ...
        if self.repre_type in ["r3m", "ag2manip", "vip", ]:
            ...
            goal_hingedoorleft_dof_pos = -1.0
            hingedoorleft_dof_pos = self.hingedoorleft_dof_pos.clone()
            
            achived_goal = (hingedoorleft_dof_pos - goal_hingedoorleft_dof_pos) < 0.

            reward = 2 * (1.0 - torch.relu(hingedoorleft_dof_pos - goal_hingedoorleft_dof_pos))
            reward_goal_bonus = 0. 
            ...
            if self.eureka_seed == 0:
                target_door_pos = -1.0
                slidedoor_dof_pos = self.hingedoorleft_dof_pos.clone()
                slidedoor_dof_vel = self.hingedoorleft_dof_vel.clone()
                # Define a scalar value for the target door position
                target_door_pos = torch.tensor(-1.0, device=slidedoor_dof_pos.device)
                # Compute the difference between the current door position and the target position
                door_pos_diff = torch.abs(slidedoor_dof_pos - target_door_pos)
                # Define a negative reward for the distance from the target position (to encourage the door to open)
                door_pos_reward = -door_pos_diff
                # Also define a negative reward for the velocity of the door (to discourage fast motion)
                door_vel_reward = -torch.abs(slidedoor_dof_vel)
                # Compute the total reward by adding up the individual components
                reward = door_pos_reward + door_vel_reward
            elif self.eureka_seed == 1:
                door_opening_reward_temp = 1.0
                vel_norm_temp = 0.1
                stop_reward_temp = 1.0
                door_goal = -1.0
                stop_vel_limit = 0.1
                slidedoor_dof_pos = self.hingedoorleft_dof_pos.clone()
                slidedoor_dof_vel = self.hingedoorleft_dof_vel.clone()
                # calculate the door opening progress, gives reward when the door is opened, normalized to [-1, 1]
                door_opening_progress = torch.clamp(slidedoor_dof_pos, min=-1, max=0) / door_goal
                door_opening_reward = torch.exp(-door_opening_reward_temp * (1 - door_opening_progress))
                # softly penalize large velocities
                vel_norm = slidedoor_dof_vel.norm(dim=-1).unsqueeze(-1)
                vel_norm_reward = torch.exp(-vel_norm_temp * vel_norm)
                # emphasize stopping at the goal
                not_stopped = torch.gt(torch.abs(slidedoor_dof_vel), stop_vel_limit).float()
                stop_reward = torch.exp(-not_stopped * stop_reward_temp)
                reward = door_opening_reward + vel_norm_reward + stop_reward
            elif self.eureka_seed == 2:
                dof_pos = self.hingedoorleft_dof_pos.clone()
                dof_vel = self.hingedoorleft_dof_vel.clone()
                goal_pos = torch.tensor(-1.0, device=dof_pos.device)
                temp_movement = torch.tensor(0.1, device=dof_pos.device)
                temp_goal = torch.tensor(0.01, device=dof_pos.device)
                # Reward for door movement towards positive direction 
                movement_reward = torch.clamp(dof_vel, min=0) / temp_movement
                # Reward when door being fully opened
                goal_reward = (torch.exp(-torch.abs(goal_pos - dof_pos)) / temp_goal) * 10000.0
                # Dictionary of reward components
                reward_dict = {}
                reward_dict['movement_reward'] = movement_reward
                reward_dict['goal_reward'] = goal_reward
                # Total reward
                reward = movement_reward + goal_reward 
            else:
                raise NotImplementedError
        else:
            raise NotImplementedError
        ...
        self.scores = torch.relu( - hingedoorleft_dof_pos)
        ...
        self.extras['indicator'] = self.hingedoorleft_dof_pos.clone().mean().unsqueeze(0)
    ...
    def compute_observations(self):
        ...
        self.hingedoorleft_dof_pos = self.dof_states[:, self.hingedoorleft_dof_handle, 0]
        self.hingedoorleft_dof_vel = self.dof_states[:, self.hingedoorleft_dof_handle, 1]
