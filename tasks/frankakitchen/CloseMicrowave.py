...
class CloseMicrowave(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        if self.is_planning or self.act_type in ['robot_joint_pose']:
            ...
            robot_start_pose.p = gymapi.Vec3(0.0, -1.2, 1.7)
            robot_start_pose.r = gymapi.Quat(*R.from_euler('xyz', [0., 0., 3.14]).as_quat())
        else:
            robot_start_pose = gymapi.Transform()
            robot_start_pose.p = gymapi.Vec3(0.0, -1.2, 2.3)
        ...
        if self.camera in [None]:
            ...
            self.camera_place_pos = gymapi.Vec3(0.1, -1., 2.5)
            self.camera_lookat_pos = gymapi.Vec3(-0.8, -1., 2.0)
        elif self.camera == 'right':
            self.camera_place_pos = gymapi.Vec3(-0.1, -0.5, 2.4)
            self.camera_lookat_pos = gymapi.Vec3(-0.6, -1., 2.2)
        elif self.camera == 'left':
            self.camera_place_pos = gymapi.Vec3(-0.1, -1.5, 2.4)
            self.camera_lookat_pos = gymapi.Vec3(-0.6, -1., 2.2)
        elif self.camera == 'demo':
            self.camera_props.width = 3840
            self.camera_props.height = 2160
            self.camera_place_pos = gymapi.Vec3(0.5, -2.5, 2.9)
        ...
        for i in range(self.num_envs):
            ...
            for object_asset_name in object_assets.keys():
                ...
                if object_asset_name == "microwave":
                    self.microdoor_dof_handle = self.gym.get_actor_dof_handle(env_ptr, object_handle, 0)
            if i == 0:
        ...
        print(f'microwave dof handle: {self.microdoor_dof_handle}')
    ...
    def compute_reward(self, actions):
        ...
        if self.repre_type in ["r3m", "ag2manip", "vip", ]:
            ...
            goal_microdoor_dof_pos = 0.0
            microdoor_dof_pos = self.microdoor_dof_pos.clone()
            achived_goal = torch.abs(goal_microdoor_dof_pos - microdoor_dof_pos) < 0.1

            reward = 1.4 * (0.7 - torch.relu(goal_microdoor_dof_pos - microdoor_dof_pos))
            reward = 1.e-3 * (torch.exp(5 * reward) - 1.0)
            reward_goal_bonus = 0. 
            ...
            if self.eureka_seed == 0:
                microwave_goal = 0.0
                microdoor_dof_pos = self.microdoor_dof_pos.clone()
                microdoor_dof_vel = self.microdoor_dof_vel.clone()
                # set temperature values
                temp_1, temp_2, temp_3 = 0.5, 1.0, 1.5
                # progress_reward calculates how much progress has been made towards closing the door (goal = 0.0)
                progress_reward = torch.exp(-torch.abs(microdoor_dof_pos - microwave_goal) * temp_1)
                # speed_penalty penalizes fast motion of the door to ensure it does not close too rapidly
                speed_penalty = 0.
                # total reward combines progress and speed penalty
                reward = progress_reward + speed_penalty
            elif self.eureka_seed == 1:
                # Beside the reaching reward, we also penalize the agent for excessive movement
                # Set the parameters
                microdoor_dof_pos = self.microdoor_dof_pos.clone()
                microdoor_dof_vel = self.microdoor_dof_vel.clone()
                goal_pos = torch.tensor(0.0, device=microdoor_dof_pos.device)
                initial_pos = torch.tensor(-0.7, device=microdoor_dof_pos.device)
                movement_penalty_temperature = torch.tensor(0.1, device=microdoor_dof_pos.device)
                # Distance from goal
                distance = torch.abs(goal_pos - microdoor_dof_pos)
                # Normalized distance from the initial position to the goal position
                normalized_distance = distance / (initial_pos - goal_pos)
                # The reward for reaching the goal is higher as the agent gets closer to it
                reach_goal_reward = 1.0 - normalized_distance
                # The penalty for movements is higher as the velocity of the door increases
                movement_penalty = torch.exp(movement_penalty_temperature * torch.abs(microdoor_dof_vel))
                # Total reward is a combination of the above components
                reward = reach_goal_reward - movement_penalty
            elif self.eureka_seed == 2:
                # Define parameters of the reward function
                door_pos_target = 0.0
                door_pos_start = -0.7
                microdoor_dof_pos = self.microdoor_dof_pos.clone()
                microdoor_dof_vel = self.microdoor_dof_vel.clone()
                door_pos_weight = torch.tensor(1.0)
                door_vel_weight = torch.tensor(0.1)
                reward_dict = {}
                # Normalize door position to range [0.0, 1.0]
                normalized_door_pos = (microdoor_dof_pos - door_pos_start) / (door_pos_target - door_pos_start)
                # Compute reward component based on door position: higher reward for being closer to the target position
                door_pos_reward = torch.where(normalized_door_pos > 1.0, torch.tensor(1.0, device=self.device), normalized_door_pos)
                reward_dict['door_pos_reward'] = door_pos_reward
                # Compute reward component based on door velocity: lower reward for higher velocity (encourages smooth movement)
                ## door_vel_reward = torch.exp(-door_vel_weight * torch.abs(microdoor_dof_vel))
                door_vel_reward = - door_vel_weight * torch.abs(microdoor_dof_vel)
                reward_dict['door_vel_reward'] = door_vel_reward
                # Compute total reward
                reward = door_pos_weight * door_pos_reward + door_vel_reward
            else:
                raise NotImplementedError
        else:
            raise NotImplementedError
        ...
        self.scores = torch.relu(microdoor_dof_pos + 0.7) / 0.7
        ...
        self.extras['indicator'] = self.microdoor_dof_pos.clone().mean().unsqueeze(0)
    ...
    def compute_observations(self):
        ...
        self.microdoor_dof_pos = self.dof_states[:, self.microdoor_dof_handle, 0]
        self.microdoor_dof_vel = self.dof_states[:, self.microdoor_dof_handle, 1]
    ...
    def _setup_attachable_body(self, ):
        ...
        for object_name in ['kettle', 'hingecabinet', 'slidecabinet', 'microwave', 'oven']:
            ...
            if object_name == 'microwave':
                object_attachs = pickle.load(open(os.path.join(env_asset_root, 'attachable', f'attach_{object_name}_door-0.7.pkl'), 'rb'))['data']
            else:
