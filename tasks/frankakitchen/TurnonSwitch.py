...
class TurnonSwitch(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        if self.is_planning or self.act_type in ['robot_joint_pose']:
            ...
            robot_start_pose.p = gymapi.Vec3(0.0, -0.6, 1.6)
            robot_start_pose.r = gymapi.Quat(*R.from_euler('xyz', [0., 0., 1.57]).as_quat())
        else:
            robot_start_pose = gymapi.Transform()
            robot_start_pose.p = gymapi.Vec3(0.0, -0.6, 2.2)
        ...
        if self.camera in [None]:
            ...
            self.camera_place_pos = gymapi.Vec3(-0.2, -0.6, 2.5)
            self.camera_lookat_pos = gymapi.Vec3(-0.2, 0.0, 2.0)
        elif self.camera == 'right':
            self.camera_place_pos = gymapi.Vec3(0.0, -0.5, 2.5)
            self.camera_lookat_pos = gymapi.Vec3(-0.2, 0.0, 2.0)
        elif self.camera == 'left':
            self.camera_place_pos = gymapi.Vec3(-0.4, -0.5, 2.5)
            self.camera_lookat_pos = gymapi.Vec3(-0.2, 0.0, 2.0)
        elif self.camera == 'demo':
            self.camera_props.width = 3840
            self.camera_props.height = 2160
            self.camera_place_pos = gymapi.Vec3(1.2, -1.2, 3.2)
            self.camera_lookat_pos = gymapi.Vec3(-1.0, 0.5, 1.5)
        else:
        ...
        print(f'slidecabinet dof handle: {self.switch_dof_handle}')
    ...
    def compute_reward(self, actions):
        ...
        if self.repre_type in ["r3m", "ag2manip", "vip", ]:
            ...
            goal_switch_dof_pos = -0.7
            switch_dof_pos = self.switch_dof_pos.clone()
            achived_goal = torch.abs(switch_dof_pos - goal_switch_dof_pos) < 0.2

            reward = 2 * (0.44 - torch.abs(switch_dof_pos - goal_switch_dof_pos))
            ...
            if self.eureka_seed == 0:
                ...
                temp = torch.tensor(0.1, device=switch_dof_pos.device)
                # compute the raw reward, i.e., the negative difference between current and goal positions
                raw_reward = -torch.abs(switch_dof_pos - goal_pos)
                # transform the raw reward using exponential function to normalize it
                trans_reward = torch.exp(temp*raw_reward)
                # compose individual reward components
                reward_components = {"raw_reward": raw_reward, "trans_reward": trans_reward}
                # compute the total reward as the mean of all reward components
                reward = torch.mean(torch.stack(list(reward_components.values())))
            elif self.eureka_seed == 1:
                goal_pos = -0.7
                switch_dof_pos = self.switch_dof_pos.clone()
                # Reward for reaching the goal position (turning the switch on)
                # We compute the absolute difference between the current switch position and the goal position,
                # and apply a negative sign to make it a minimization problem.
                # We reduce it with mean to get a scalar value.
                reward_goal = -torch.mean(torch.abs(switch_dof_pos - goal_pos))
                # Encourage the gripper to move towards the goal position by providing a direction hint
                # The more aligned the movement direction with the goal direction, the higher this reward will be
                reward_direction = torch.mean(torch.tensor(1.0, device=switch_dof_pos.device) - torch.abs(switch_dof_pos - goal_pos))
                # The total reward is a weighted sum of the sub-rewards
                reward = 0.7 * reward_goal + 0.3 * reward_direction
            elif self.eureka_seed == 2:
                switch_goal_pos = -0.7
                switch_dof_pos = self.switch_dof_pos.clone()
                # Initialize the distance between the switch's current position and the goal
                distance = torch.abs(switch_dof_pos - switch_goal_pos)
                # Set temperature
                temp = torch.tensor(0.1).to(distance.device)
                # Compute reward
                reward_distance = torch.exp(-temp * distance)  # Encourages the agent to get close to the goal
                reward_goal = (switch_goal_pos <= switch_dof_pos).float()  # Extra reward if the goal is reached
                # Compute total reward
                reward = reward_distance + reward_goal
            else:
                raise NotImplementedError
        else:
            raise NotImplementedError
        ...
        self.scores = switch_dof_pos.clone() / -0.7
        ...
        self.extras['indicator'] = self.switch_dof_pos.clone().mean().unsqueeze(0)
    ...
    def compute_observations(self):
        ...
        self.switch_dof_pos = self.dof_states[:, self.switch_dof_handle, 0]
        self.switch_dof_vel = self.dof_states[:, self.switch_dof_handle, 1]
