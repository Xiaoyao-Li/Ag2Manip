...
class CloseSlidecabinet(BaseTask):
    ...
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, camera, headless):
        ...
        if self.act_type in ['robot_joint_pose']:
            self.num_obs_dict = {
                "robot_state": 8 * 2 + 1,
                "full_state": 28 * 2 + 13 + 1 + 1,
            }
        elif self.act_type in ['dummy_interaction', 'dummy_interaction_sphere', 'end_effector_pose']:
            self.num_obs_dict = {
                "robot_state": 8 * 2 + 1,
                "full_state": 22 * 2 + 13 + 1 + 1,  
            }
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        if self.is_planning or self.act_type in ['robot_joint_pose']:
            ...
            robot_start_pose.p = gymapi.Vec3(-0.0, -0.6, 2.0)
            robot_start_pose.r = gymapi.Quat(*R.from_euler('xyz', [0., 0., 1.57]).as_quat())
        else:
            robot_start_pose = gymapi.Transform()
            robot_start_pose.p = gymapi.Vec3(-0.0, -0.6, 2.6)
        ...
        for object_asset_name in ["floor", "backwall", "hingecabinet", "kettle", "microwave", "oven", "slidecabinet", "room_0", "room_1"]:
        ...
        if self.camera in [None]:
            ...
            self.camera_place_pos = gymapi.Vec3(0.0, -1.1, 2.7)
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.0, 2.0)
        elif self.camera == 'right':
            self.camera_place_pos = gymapi.Vec3(0.5, -0.9, 2.7)
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.0, 2.2)
            ...
            self.camera_place_pos = gymapi.Vec3(1.2, -1.5, 2.7)
            self.camera_lookat_pos = gymapi.Vec3(-0.2, -0.5, 2.1)
        else:
        ...
        for i in range(self.num_envs):
            ...
            for object_asset_name in object_assets.keys():
                ...
                # set joint dof statr states
                if object_asset_name == "slidecabinet":
                    dof_states = self.gym.get_actor_dof_states(env_ptr, object_handle, gymapi.STATE_ALL)
                    dof_states['pos'][0] = 0.44
                    dof_states['vel'][0] = 0.0

                    self.gym.set_actor_dof_states(env_ptr, object_handle, dof_states, gymapi.STATE_ALL)

                # get actor joint names
                if object_asset_name == "slidecabinet":
                    self.slidedoor_dof_handle = self.gym.get_actor_dof_handle(env_ptr, object_handle, 0)
                    self.slidedoor_body_handle = self.gym.get_actor_rigid_body_handle(env_ptr, object_handle, 2)
        ...
        print(f'slidecabinet dof handle: {self.slidedoor_dof_handle}')
        print(f'slidecabinet body handle: {self.slidedoor_body_handle}')
    ...
    def compute_reward(self, actions):
        ...
        if self.repre_type in ["r3m", "ag2manip", "vip", ]:
            ...
            goal_slidedoor_dof_pos = 0.0
            slidedoor_dof_pos = self.slidedoor_dof_pos.clone()
            
            achived_goal = torch.abs(slidedoor_dof_pos - goal_slidedoor_dof_pos) < 0.04

            reward = 2 * (0.44 - torch.abs(slidedoor_dof_pos - goal_slidedoor_dof_pos))
            reward_goal_bonus = 0. 

            reward = torch.where(achived_goal, reward + reward_goal_bonus, reward)
        
        elif self.repre_type in ['eureka']:
            if self.eureka_seed == 0:
                goal_pos = 0.0
                slidedoor_dof_pos = self.slidedoor_dof_pos.clone()
                # calculate the distance between the current gripper position and the goal
                # distance_to_target = torch.norm(gripper_pos - goal_pos, dim=-1)
                # calculate the error in door position, the desired door position is 0
                door_error = torch.abs(slidedoor_dof_pos)
                # calculate the total reward as the negative sum of distance_to_target and door_error
                reward = - door_error
            elif self.eureka_seed == 1:
                slidedoor_pos = self.slidedoor_dof_pos.clone()
                slidedoor_dof_vel = self.slidedoor_dof_vel.clone()
                goal_pos = torch.tensor(0.0).to(slidedoor_pos.device)
                initial_pos = torch.tensor(0.44).to(slidedoor_pos.device)
                # Calculate the normalized distance from goal
                dist_to_goal = torch.abs(slidedoor_pos - goal_pos) / (initial_pos - goal_pos)
                # Calculate the reward for being close to goal
                reward_dist = -dist_to_goal
                # Encourage slow closing for stability; punish fast movements
                reward_vel = -torch.abs(slidedoor_dof_vel)
                # Bonus reward for reaching the goal
                is_at_goal = torch.abs(slidedoor_pos - goal_pos) < 0.01
                reward_goal = is_at_goal.float()
                # Encourage stability by rewarding low linear and angular velocities
                reward = reward_dist + reward_vel + reward_goal
            elif self.eureka_seed == 2:
                goal_pos = torch.tensor(0.0).to(self.slidedoor_dof_pos.device)
                slidedoor_pos = self.slidedoor_dof_pos.clone()
                # Define a temperature parameter for the transformed reward component
                temperature: torch.Tensor = torch.tensor([0.1]).to(slidedoor_pos.device)
                # Define a dictionary to hold each individual reward component
                individual_reward_components = {}
                # Compute the difference between current slidedoor position and the goal position
                position_diff: torch.Tensor = torch.abs(slidedoor_pos - goal_pos)
                # Define a reward for getting the slidedoor closer to the goal
                reward_closer_to_goal: torch.Tensor = -position_diff
                # Add reward_closer_to_goal to the dictionary of individual reward components
                individual_reward_components['reward_closer_to_goal'] = reward_closer_to_goal
                # Normalize the reward_closer_to_goal using an exponential function and the temperature parameter
                reward_closer_to_goal_normalized: torch.Tensor = torch.exp(reward_closer_to_goal / temperature)
                # Define the total reward as the normalized reward_closer_to_goal
                reward: torch.Tensor = reward_closer_to_goal_normalized
            else:
                raise NotImplementedError
        else:
            raise NotImplementedError
        ...
        self.scores = (0.44 - slidedoor_dof_pos.clone()) / 0.44
        ...
        self.extras['indicator'] = self.slidedoor_dof_pos.clone().mean().unsqueeze(0)
    ...
    def compute_observations(self):
        ...
        self.kettle_pose = self.root_state_tensor[:, self.objects_handle["kettle"], 0:7]
        self.kettle_pos = self.root_state_tensor[:, self.objects_handle["kettle"], 0:3]
        self.kettle_rot = self.root_state_tensor[:, self.objects_handle["kettle"], 3:7]
        self.kettle_linvel = self.root_state_tensor[:, self.objects_handle["kettle"], 7:10]
        self.kettle_angvel = self.root_state_tensor[:, self.objects_handle["kettle"], 10:13]

        self.slidedoor_pose = self.rigid_body_states[:, self.slidedoor_body_handle, 0:7]
        self.slidedoor_pos = self.rigid_body_states[:, self.slidedoor_body_handle, 0:3]
        self.slidedoor_rot = self.rigid_body_states[:, self.slidedoor_body_handle, 3:7]
        self.slidedoor_linvel = self.rigid_body_states[:, self.slidedoor_body_handle, 7:10]
        self.slidedoor_angvel = self.rigid_body_states[:, self.slidedoor_body_handle, 10:13]

        self.slidedoor_dof_pos = self.dof_states[:, self.slidedoor_dof_handle, 0]
        self.slidedoor_dof_vel = self.dof_states[:, self.slidedoor_dof_handle, 1]
        # print(f'slidecabinet dof pos: {self.slidedoor_dof_pos}') # shape: [num_envs]
        # print(f'slidecabinet dof vel: {self.slidedoor_dof_vel}') # shape: [num_envs]
    ...
    def compute_full_state(self):
        ...
        0 - 8       robot joint dofs position
        9 - 17      robot joint dofs velocity
        18 - 19     slidecabinet dof position
        19 - 20     slidecabient dof velocity
        """
        ...
        self.obs_buf[:, 2*self.num_dofs : 2*self.num_dofs + 7] = self.kettle_pose
        self.obs_buf[:, 2*self.num_dofs + 7 : 2*self.num_dofs + 10] = self.kettle_linvel
        self.obs_buf[:, 2*self.num_dofs + 10 : 2*self.num_dofs + 13] = self.kettle_angvel
    ...
    def _setup_attachable_body(self, ):
        ...
        for object_name in ['kettle', 'hingecabinet', 'slidecabinet', 'microwave', 'oven']:
            ## load attach info
            if object_name == 'slidecabinet':
                object_attachs = pickle.load(open(os.path.join(env_asset_root, 'attachable', f'attach_{object_name}_door0.44.pkl'), 'rb'))['data']
            else:
