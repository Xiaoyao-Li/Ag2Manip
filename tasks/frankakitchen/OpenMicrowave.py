...
class OpenMicrowave(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        if self.is_planning or self.act_type in ['robot_joint_pose']:
            ...
            robot_start_pose.p = gymapi.Vec3(0.0, -1.0, 1.7)
            robot_start_pose.r = gymapi.Quat(*R.from_euler('xyz', [0., 0., 3.14]).as_quat())
        else:
            robot_start_pose = gymapi.Transform()
            robot_start_pose.p = gymapi.Vec3(0.0, -1.0, 2.3)
    ...
    def compute_reward(self, actions):
        ...
        if self.repre_type in ["r3m", "ag2manip", "vip", ]:
            ...
            if self.eureka_seed == 0:
                ...
                goal_pos = -0.7
                microdoor_dof_pos = self.microdoor_dof_pos.clone()
                device = microdoor_dof_pos.device
                # Goal reach reward
                threshold = torch.tensor([0.01], device=device)  # you can adjust this threshold
                reached_goal = torch.where(torch.abs(microdoor_dof_pos - goal_pos) < threshold, 1.0, 0.0)
                reach_reward = torch.where(reached_goal > 0, 1.0, -1.0)
                # Distance reward
                goal_dist = torch.abs(microdoor_dof_pos - goal_pos)
                dist_reward = -1.0 * goal_dist  # the closer to the goal, the higher the reward
                # Total reward is combination of reach and distance rewards
                reward = reach_reward + dist_reward
            elif self.eureka_seed == 1:
                # reward components
                goal_pos = -0.7
                microdoor_dof_pos = self.microdoor_dof_pos.clone()
                microdoor_dof_vel = self.microdoor_dof_vel.clone()
                distance_to_goal = torch.abs(goal_pos - microdoor_dof_pos)
                velocity_penalty = torch.abs(microdoor_dof_vel)
                # reward transformation parameters
                distance_temp = 0.05
                velocity_temp = 0.1
                # reward transformation
                distance_reward = torch.exp(-distance_temp * distance_to_goal)
                ...
                microdoor_dof_pos = self.microdoor_dof_pos.clone()
                microdoor_dof_vel = self.microdoor_dof_vel.clone()
                goal_pos = torch.tensor(-0.7).to(microdoor_dof_pos.device)
                action_penalty_temp = torch.tensor(0.5).to(microdoor_dof_pos.device)
                distance_penalty_temp = torch.tensor(0.5).to(microdoor_dof_pos.device)
                # Reward based on the distance to the goal
                distance_reward = -torch.abs(goal_pos - microdoor_dof_pos)
                # Penalty for taking actions (to encourage efficiency), use sigmoid to map velocity [-1, 1] to penalty [0, 1]
                action_penalty = -torch.exp(-action_penalty_temp * torch.sigmoid(microdoor_dof_vel))
                # Apply transformation to distance reward
                distance_reward_transformed = torch.exp(distance_penalty_temp * distance_reward)
                reward = distance_reward_transformed + action_penalty
            else:
                raise NotImplementedError
        else:
            raise NotImplementedError
        ...
        self.scores = torch.relu(- microdoor_dof_pos) / 0.7
