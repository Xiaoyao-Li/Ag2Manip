...
class StackCube(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for object_asset_name in ["floor", "room_0", "room_1", 'cube', 'basecube', 'table']:
    ...
    def compute_reward(self, actions):
        ...
        if self.repre_type in ["r3m", "ag2manip", "vip", ]:
            ...
            goal_cube_pose = torch.tensor([0.1, 0.0, 1.284, 0., 0., 0., 1.], device=self.device)
            cube_pose = self.cube_pose.clone()
            achived_goal = torch.norm(cube_pose[..., :3] - goal_cube_pose[:3], dim=-1) < 0.05

            reward = 1 - torch.norm(cube_pose[..., :3] - goal_cube_pose[:3], dim=-1) / 0.206
            reward = torch.where(reward < 0, torch.exp(reward) - 1, 10 * (torch.exp(2 * reward) - 1))
        
        elif self.repre_type in ['eureka']:
            if self.eureka_seed == 0:
                goal_pos = torch.tensor([0.1, 0.0, 1.284], device=self.device)
                cube_pose = self.cube_pose.clone()[:, :3]
                cube_linvel = self.cube_linvel.clone()
                cube_angvel = self.cube_angvel.clone()
                # Density and temperature parameters
                dist_den = 0.05
                vel_den = 0.1
                ang_den = 0.1
                cube_reach_temp = 5.0
                cube_stable_temp = 1.0
                # Reward components
                cube_dist = cube_pose - goal_pos
                cube_reach_reward = torch.exp(-cube_reach_temp * (cube_dist.norm(dim=-1) / dist_den))  # Cube reaching reward
                cube_stable_reward = torch.exp(-cube_stable_temp* ((cube_linvel.norm(dim=-1) + cube_angvel.norm(dim=-1)) / (vel_den + ang_den)))  # Cube stability reward
                # Total reward
                reward = cube_reach_reward + cube_stable_reward
            elif self.eureka_seed == 1:
                goal_pos = torch.tensor([0.1, 0.0, 1.284], device=self.device)
                cube_pose = self.cube_pose.clone()[:, :3]
                # Temperature parameter for reward transformations
                reward_temp = torch.tensor(0.1).to(goal_pos.device)
                # Calculate the distance between the cube and the goal
                dist = torch.norm(cube_pose - goal_pos)
                # Write a reward function that encourages the agent to move the cube closer to the goal
                reward_dist = -dist
                reward_dist_exp = torch.exp(reward_temp * reward_dist)
                # Check if the cube is placed at the goal pos
                # cube_at_goal = torch.tensor(float(cube_pose == goal_pos)).to(goal_pos.device)
                # reward_goal = cube_at_goal * reward_dist_exp
                # Combine individual rewards into a total reward
                reward = reward_dist_exp
            elif self.eureka_seed == 2:
                goal_pos = torch.tensor([0.1, 0.0, 1.284], device=self.device)
                cube_pose = self.cube_pose.clone()[:, :3]
                # namedtuple to hold the reward components                
                # Positional error is the euclidean distance between the current and goal positions
                pos_error = torch.norm(cube_pose - goal_pos, dim=-1)
                # Reward for minimizing positional error (distance to the goal)
                # Using an exponential function for the reward emphasizes reaching the goal quicker
                dist_reward_temp = 0.1
                dist_reward = torch.exp(-dist_reward_temp * pos_error)
                # Reward for minimizing cube's linear and angular velocities
                # This encourages the agent to achieve a stable configuration once the cube is at the goal position
                vel_reward_temp = 0.2
                cube_vel_error = torch.norm(self.cube_linvel, dim=-1) + torch.norm(self.cube_angvel, dim=-1)
                vel_reward = torch.exp(-vel_reward_temp * cube_vel_error)
                # Total reward is the sum of component rewards
        ...
        self.scores = 1 - torch.norm(cube_pose[:, :3] - goal_cube_pose[:3], dim=1) / 0.2
