...
class InsertPeg(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for object_asset_name, object_asset_file in object_asset_files.items():
            ...
            if object_asset_name == 'stick':
                object_asset_options.fix_base_link = False
                object_asset_options.disable_gravity = True
            if object_asset_name == 'holebox':
                object_asset_options.fix_base_link = False
        ...
        if self.is_planning or self.act_type in ['robot_joint_pose']:
            ...
            robot_start_pose.p = gymapi.Vec3(0.0, -1.0, 1.5)
            robot_start_pose.r = gymapi.Quat(*R.from_euler('xyz', [0., 0., 1.57]).as_quat())
        else:
            robot_start_pose = gymapi.Transform()
            robot_start_pose.p = gymapi.Vec3(0.0, -0.4, 1.7)
        ...
        for object_asset_name in ["floor", "room_0", "room_1", 'stick', 'holebox', 'table']:
        ...
        if self.camera in [None]:
            ...
            self.camera_place_pos = gymapi.Vec3(0.0, -0.4, 1.5)
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.4, 1.2)
        elif self.camera == 'right':
            self.camera_place_pos = gymapi.Vec3(0.4, -0.4, 1.5)
            self.camera_lookat_pos = gymapi.Vec3(-0.3, 0.4, 1.2)
        ...
        for i in range(self.num_envs):
            ...
            for object_asset_name in object_assets.keys():
                ...
                if object_asset_name == 'stick':
                    self.stick_body_handle = self.gym.get_actor_rigid_body_handle(env_ptr, object_handle, 0)
    ...
    def compute_reward(self, actions):
        ...
        if self.repre_type in ["r3m", "ag2manip", "vip", ]:
            ...
            goal_stick_pose = torch.tensor([0.04, -0.043, 1.308, 0, 0, 0.26, 0.97], device=self.device)
            stick_pose = self.stick_pose.clone()
            achived_goal = torch.norm(stick_pose[..., :3] - goal_stick_pose[:3], dim=-1) < 0.05

            reward = 4 * (0.2158 - torch.norm(stick_pose[..., :3] - goal_stick_pose[:3], dim=-1))
            reward = torch.where(reward < 0, torch.exp(reward) - 1, 10 * (torch.exp(2 * reward) - 1))
        
        elif self.repre_type in ['eureka']:
            if self.eureka_seed == 0:
                stick_pose = self.stick_pose.clone()[:, :3]
                goal_pos = torch.tensor([0.04, -0.043, 1.308], device=stick_pose.device)
                # Compute the distance between the stick and the goal position
                distance_to_goal = torch.norm(stick_pose[:, :3] - goal_pos, dim=-1)
                # Define the reward components
                reward_insert = -distance_to_goal
                total_reward = reward_insert
                # Normalize the reward with a temperature parameter.
                temperature = torch.tensor(0.1).to(stick_pose.device)
                total_reward /= temperature
                reward = total_reward
            elif self.eureka_seed == 1:
                stick_pos = self.stick_pose.clone()[:, :3]
                stick_linvel = self.stick_linvel.clone()
                goal_pos = torch.tensor([0.04, -0.043, 1.308], device=stick_pos.device)
                initial_pos = torch.tensor([-0.15, -0.1, 1.223], device=stick_pos.device)
                # Compute distance to the goal
                dist_to_goal = torch.norm(stick_pos - goal_pos, dim=-1)
                initial_dist_to_goal = torch.norm(initial_pos - goal_pos)
                # Compute the reward for approaching the goal
                reward_dist = torch.exp(-torch.abs(dist_to_goal / initial_dist_to_goal))
                # Compute velocity direction
                vel_dir = torch.sum((stick_linvel / (torch.norm(stick_linvel, dim=-1, keepdim=True) + 1e-8)) * (goal_pos - stick_pos), dim=-1)
                # Compute reward for velocity direction
                reward_vel = torch.clamp(vel_dir, min=0, max=1)
                # Compute the total reward
                reward = 0.5 * (reward_dist + reward_vel)
            elif self.eureka_seed == 2:
                stick_pose = self.stick_pose.clone()[:, :3]
                goal_pos_tensor = torch.tensor([0.04, -0.043, 1.308], device=stick_pose.device)
                initial_pos_tensor = torch.tensor([-0.15, -0.1, 1.223], device=stick_pose.device)
                # Distance from the current position to the goal
                dist_to_goal = torch.norm(stick_pose - goal_pos_tensor)
                # Distance from the initial position to the goal
                initial_dist_to_goal = torch.norm(initial_pos_tensor - goal_pos_tensor)
                # Reward is higher the closer the stick is to the goal
                reward_dist_to_goal = -torch.exp(dist_to_goal)
                # Transforming the rewards with a temperature parameter
                temperature = torch.tensor(0.1, device=stick_pose.device)
                reward_dist_to_goal = torch.exp(reward_dist_to_goal / temperature)
                # The total reward is a combination of the individual rewards
                reward = reward_dist_to_goal
            else:
                raise NotImplementedError
        else:
            raise NotImplementedError
        ...
        self.scores = 1 - torch.norm(stick_pose[:, :3] - goal_stick_pose[:3], dim=1) / 0.2158
        ...
        self.extras['indicator'] = self.stick_pose.clone().mean(dim=0)
    ...
    def compute_observations(self):
        ...
        self.stick_pose = self.rigid_body_states[:, self.stick_body_handle, 0:7]
        self.stick_linvel = self.rigid_body_states[:, self.stick_body_handle, 7:10]
        self.stick_angvel = self.rigid_body_states[:, self.stick_body_handle, 10:13]
    ...
    def _setup_attachable_body(self, ):
        ...
        for object_name in ['stick', 'holebox']:
            ## load attach info
