...
class CloseDoor(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        if self.is_planning or self.act_type in ['robot_joint_pose']:
            ...
            robot_start_pose = gymapi.Transform()
            robot_start_pose.p = gymapi.Vec3(0.0, -0.7, 2.1)
        ...
        for object_asset_name in ["floor", "room_0", "room_1", 'doorcabinet', 'table']:
        ...
        if self.camera in [None]:
            ...
            self.camera_place_pos = gymapi.Vec3(0.0, -1.0, 2.0)
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.0, 1.5)
        elif self.camera == 'right':
            self.camera_place_pos = gymapi.Vec3(0.4, -0.9, 2.0)
            self.camera_lookat_pos = gymapi.Vec3(-0.1, 0.0, 1.5)
            ...
            self.camera_place_pos = gymapi.Vec3(1.5, -1.7, 2.5)
        ...
        for i in range(self.num_envs):
            ...
            for object_asset_name in object_assets.keys():
                ...
                if object_asset_name == 'doorcabinet':
                    self.doorcabinet_dof_handle = self.gym.find_actor_dof_handle(env_ptr, object_handle, 'joint_0')
    ...
    def compute_reward(self, actions):
        ...
        if self.repre_type in ["r3m", "ag2manip", "vip", ]:
            ...
            goal_doorcabinet_dof_pos = 0.0
            doorcabinet_dof_pos = self.doorcabinet_dof_pos.clone()
            achived_goal = torch.abs(goal_doorcabinet_dof_pos - doorcabinet_dof_pos) < 0.2

            reward = 1.0 - doorcabinet_dof_pos
            reward = torch.where(reward < 0, torch.exp(reward) - 1, 10 * (torch.exp(2 * reward) - 1))
        
        elif self.repre_type in ['eureka']:
            if self.eureka_seed == 0:
                doorcabinet_dof_pos = self.doorcabinet_dof_pos.clone()
                doorcabinet_dof_vel = self.doorcabinet_dof_vel.clone()
                goal_dof_pos = torch.zeros_like(doorcabinet_dof_pos).to(doorcabinet_dof_pos.device)
                goal_dof_vel = torch.zeros_like(doorcabinet_dof_vel).to(doorcabinet_dof_vel.device)
                # closer to the goal is better
                dist_reward = -torch.abs(doorcabinet_dof_pos - goal_dof_pos)
                # slower is better
                vel_reward = -torch.abs(doorcabinet_dof_vel - goal_dof_vel)
                # temperature vars for reward transformation
                temp_dist = 1.0
                temp_vel = 1.0
                dist_reward = temp_dist * torch.exp(dist_reward)
                vel_reward = temp_vel * torch.exp(vel_reward)
                # whether the door is fully closed
                closed_reward = (doorcabinet_dof_pos <= 0.01).float()
                # total reward
                reward = dist_reward + closed_reward
            elif self.eureka_seed == 1:
                goal_dof = 0.0
                door_dof = self.doorcabinet_dof_pos.clone()
                # Introduce a temperature to dampen the reward fluctuations
                reward_temperature = torch.tensor(0.1, device=door_dof.device)
                # Difference between the door_dof and the goal_dof
                dof_difference = torch.abs(door_dof - goal_dof)
                # Compute the reward: negative reward as the dof_difference from the goal increases. We clamp it to keep rewards in the range -1 to 1
                reward = torch.clamp(-1. * dof_difference / reward_temperature, -1., 1.)
            elif self.eureka_seed == 2:
                goal_pos = 0.0
                doorcabinet_dof_pos = self.doorcabinet_dof_pos.clone()
                # Distance from the current position of the door to the goal
                distance_to_goal = torch.abs(doorcabinet_dof_pos - goal_pos)
                # Reward for reaching the target position
                goal_reached_reward = torch.where(doorcabinet_dof_pos == goal_pos, torch.tensor(1.0).to(doorcabinet_dof_pos.device), torch.tensor(0.0).to(doorcabinet_dof_pos.device))
                # Combining rewards and normalizing them
                reward = -(distance_to_goal) + goal_reached_reward
            else:
                raise NotImplementedError
        else:
            raise NotImplementedError
        ...
        self.scores = 1.0 - doorcabinet_dof_pos
        ...
        self.extras['indicator'] = self.doorcabinet_dof_pos.mean().unsqueeze(0)
    ...
    def compute_observations(self):
        ...
        self.doorcabinet_dof_pos = self.dof_states[:, self.doorcabinet_dof_handle, 0]
        self.doorcabinet_dof_vel = self.dof_states[:, self.doorcabinet_dof_handle, 1]
    ...
    def _setup_attachable_body(self, ):
        ...
        for object_name in ['doorcabinet']:
            ## load attach info
            if object_name == 'doorcabinet':
                object_attachs = pickle.load(open(os.path.join(env_asset_root, 'attachable', f'attach_{object_name}_door1.0.pkl'), 'rb'))['data']
            else:
