...
class TurnRightfaucet(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for i in range(self.num_envs):
            ...
            for object_asset_name in object_assets.keys():
                ...
                if object_asset_name == 'rightfaucet':
                    self.rightfaucet_dof_handle = self.gym.find_actor_dof_handle(env_ptr, object_handle, 'joint_turn')
    ...
    def compute_reward(self, actions):
        ...
        if self.repre_type in ["r3m", "ag2manip", "vip", ]:
            ...
            goal_rightfaucet_dof_pos = 1.0
            rightfaucet_dof_pos = self.rightfaucet_dof_pos.clone()
            achived_goal = torch.abs(goal_rightfaucet_dof_pos - rightfaucet_dof_pos) < 0.2

            reward = rightfaucet_dof_pos / goal_rightfaucet_dof_pos
            reward = torch.where(reward < 0, torch.exp(reward) - 1, 10 * (torch.exp(2 * reward) - 1))
        
        elif self.repre_type in ['eureka']:
            if self.eureka_seed == 0:
                rightfaucet_dof_pos = self.rightfaucet_dof_pos.clone()
                goal_pos = torch.tensor([1.0], dtype=torch.float, device=self.device)
                # Initialize a dictionary for individual reward components
                reward_components = {}
                # Compute the position error between faucet position and goal
                pos_error = torch.abs(rightfaucet_dof_pos - goal_pos)
                # Assign a reward based on the inverse of error; more the error, lesser the reward
                pos_reward = 1.0 / (1.0 + pos_error)
                # store the reward components
                reward_components["pos"] = pos_reward
                # The total reward is the sum of all reward components
                reward = pos_reward
            elif self.eureka_seed == 1:
                rightfaucet_dof_pos = self.rightfaucet_dof_pos.clone()
                goal_pos = torch.tensor([1.0], dtype=torch.float, device=self.device)
                reward_distance = -torch.abs(rightfaucet_dof_pos - goal_pos)
                # Introduce a temperature parameter to tune the transformation function
                temperature_distance = torch.tensor(0.1).to(rightfaucet_dof_pos.device)
                reward_distance_exp = torch.exp(reward_distance / temperature_distance)
                reward = reward_distance_exp
            elif self.eureka_seed == 2:
                rightfaucet_dof_vel = self.rightfaucet_dof_vel.clone()
                rightfaucet_dof_pos = self.rightfaucet_dof_pos.clone()
                pos_reward_temperature = 0.1
                target_pos = torch.Tensor([1.0]).to(rightfaucet_dof_pos.device)  
                current_pos = rightfaucet_dof_pos.clone().detach()
                pos_error = torch.abs(target_pos - current_pos)
                pos_reward = torch.exp(-pos_reward_temperature * pos_error)
                vel_reward_temperature = 0.05
                target_vel = torch.Tensor([0.0]).to(rightfaucet_dof_vel.device)  
                current_vel = rightfaucet_dof_vel.clone().detach()
                vel_error = torch.abs(target_vel - current_vel)
                vel_reward = torch.exp(-vel_reward_temperature * vel_error)
        ...
        self.scores = rightfaucet_dof_pos / goal_rightfaucet_dof_pos
        ...
        self.extras['indicator'] = self.rightfaucet_dof_pos.clone().mean().unsqueeze(0)
    ...
    def compute_observations(self):
        ...
        self.rightfaucet_dof_pos = self.dof_states[:, self.rightfaucet_dof_handle, 0]
        self.rightfaucet_dof_vel = self.dof_states[:, self.rightfaucet_dof_handle, 1]
