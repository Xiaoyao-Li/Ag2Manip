...
class OpenDoor(BaseTask):
    ...
    def compute_reward(self, actions):
        ...
        if self.repre_type in ["r3m", "ag2manip", "vip", ]:
            ...
            reward = doorcabinet_dof_pos / goal_doorcabinet_dof_pos
            reward = torch.where(reward < 0, torch.exp(reward) - 1, 10 * (torch.exp(2 * reward) - 1))
            ...
            if self.eureka_seed == 0:
                doorcabinet_dof_pos = self.doorcabinet_dof_pos.clone()
                goal_pos = 1.0
                # Define the temperature variable for reward transformation
                temperature: float = 0.1
                # Compute the distance between the current door dof and goal dof
                distance = torch.abs(doorcabinet_dof_pos - goal_pos)
                # Define the reward for being close to the goal dof
                reward_goal = torch.exp(-temperature * distance)
                # Compute the total reward
                reward = reward_goal
            elif self.eureka_seed == 1:
                doorcabinet_dof_pos = self.doorcabinet_dof_pos.clone()
                doorcabinet_dof_vel = self.doorcabinet_dof_vel.clone()
                # Define the desired goal position for the door
                goal_pos = torch.tensor(1.0, device=doorcabinet_dof_pos.device)
                # Define the initial position for the door
                init_pos = torch.tensor(0.0, device=doorcabinet_dof_pos.device)
                # Calculate the distance to the goal from the current position
                dist_to_goal = torch.abs(goal_pos - doorcabinet_dof_pos)
                # Reward for reaching the goal (open the door)
                goal_reached = torch.where(dist_to_goal < 0.01, torch.tensor(100.0, device=doorcabinet_dof_pos.device), torch.tensor(0.0, device=doorcabinet_dof_pos.device))
                # Penalize for moving the door too fast
                speed_penalty = torch.where(torch.abs(doorcabinet_dof_vel) > 0.1, torch.tensor(-1.0, device=doorcabinet_dof_pos.device), torch.tensor(0.0, device=doorcabinet_dof_pos.device))
                # Total reward is the sum of the reward components
                reward = goal_reached + speed_penalty
            elif self.eureka_seed == 2:
                target_pos = 1.0
                doorcabinet_dof_pos = self.doorcabinet_dof_pos.clone()
                # The reward is computed as the negative distance between current door position and the target door position.
                distance = target_pos - doorcabinet_dof_pos
                # We normalize the reward to [-1, 0] due to the ranges of doorcabinet_dof_pos and target_pos.
                normalized_reward = -torch.abs(distance)
                # Applying an exponential transformation to the reward
                # Introducing a temperature parameter inside the transformation function
                temp: float = 0.01
                reward = torch.exp(normalized_reward / temp)
            else:
                raise NotImplementedError
        else:
            raise NotImplementedError
        ...
        self.scores = doorcabinet_dof_pos / goal_doorcabinet_dof_pos
        ...
        self.extras['indicator'] = self.doorcabinet_dof_pos.clone().mean().unsqueeze(0)
