...
class PickupClutterycb(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for object_asset_name, object_asset_file in object_asset_files.items():
            ...
            if object_asset_name in ['apple', 'fork', 'plate', 'sugar_box', 'tomato_soup_can', 'mug']:
        ...
        for object_asset_name in ["floor", "room_0", "room_1", 'table',
                                  'apple', 'fork', 'plate', 'sugar_box', 'tomato_soup_can', 'mug',]:
        ...
        if self.camera in [None]:
            ...
            self.camera_place_pos = gymapi.Vec3(0.0, -0.5, 1.4)
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.4, 1.3)
        elif self.camera == 'right':
            self.camera_place_pos = gymapi.Vec3(0.3, -0.3, 1.4)
            self.camera_lookat_pos = gymapi.Vec3(-0.4, 0.2, 1.3)
        elif self.camera == 'left':
            self.camera_place_pos = gymapi.Vec3(-0.4, -0.4, 1.4)
            self.camera_lookat_pos = gymapi.Vec3(0.4, 0.4, 1.3)
        elif self.camera == 'demo':
            self.camera_props.width = 3840
            self.camera_props.height = 2160
            self.camera_place_pos = gymapi.Vec3(0.8, 1.5, 2.1)
            self.camera_lookat_pos = gymapi.Vec3(-0.8, -0.5, 1.6)
        else:
        ...
        for i in range(self.num_envs):
            ...
            for object_asset_name in object_assets.keys():
                ...
                if object_asset_name == 'mug':
                    self.mug_body_handle = self.gym.get_actor_rigid_body_handle(env_ptr, object_handle, 0)
    ...
    def compute_reward(self, actions):
        ...
        if self.repre_type in ["r3m", "ag2manip", "vip", ]:
            ...
            goal_mug_pose = torch.tensor([-0.2, -0.2, 1.4, 0., 0., 0., 1.], device=self.device)
            mug_pose = self.mug_pose.clone()
            achived_goal = torch.norm(mug_pose[..., :3] - goal_mug_pose[:3], dim=-1) < 0.05

            reward = (0.151 - torch.norm(mug_pose[..., :3] - goal_mug_pose[:3], dim=-1)) / 0.151
            reward = torch.where(reward < 0, torch.exp(reward) - 1, 10 * (torch.exp(2 * reward) - 1))
        
        elif self.repre_type in ['eureka']:
            if self.eureka_seed == 0:
                mug_pose = self.mug_pose.clone()[:, :3]
                goal_pos = torch.tensor([-0.2, -0.2, 1.4], device=self.device)
                # Define temperature parameter
                temp_distance = torch.tensor(0.1)
                # Compute distance to the goal
                distance_to_goal = torch.sqrt(torch.sum((mug_pose-goal_pos)**2, dim=-1))
                # reward is higher when the distance to the goal is closer to 0
                reward_distance = torch.exp(-temp_distance * distance_to_goal)
                # reward for being above y=1.249 (mug initial z position)
                reward_height = torch.where(mug_pose[:, 2] > 1.249, torch.tensor(1.0), torch.tensor(0.0))
                # total reward
                reward = reward_distance * reward_height
            elif self.eureka_seed == 1:
                mug_pose = self.mug_pose.clone()[:, :3]
                goal_pos = torch.tensor([-0.2, -0.2, 1.4], device=self.device)
                at_goal_reward_coef = 1000.0
                at_goal_margin = 0.05
                dist_coef = 1.0
                goal_pos_tensor = torch.tensor(goal_pos, device=mug_pose.device)
                # calculations
                dist_to_goal = torch.norm(mug_pose[:, :3] - goal_pos_tensor, dim=1)
                # rewards
                at_goal_reward = (dist_to_goal < at_goal_margin).float() * at_goal_reward_coef
                dist_reward = -dist_coef * dist_to_goal
                reward = at_goal_reward + dist_reward
            elif self.eureka_seed == 2:
                target_pos = torch.tensor([-0.2, -0.2, 1.4], device=self.device)
                mug_pose = self.mug_pose.clone()[:, :3]
                # initialize the results
                reward_dict = {}
                # Get the device f the tensor
                device = mug_pose.device
                # calculate distance from current mug position to target position
                distance = torch.linalg.norm(target_pos - mug_pose, dim=-1)
                reward_dict["distance_to_target"] = -distance
                # give a reward when the mug is lifted
                lifted = torch.where(mug_pose[:, 2] > 1.3, torch.tensor([1.0], device=device), torch.tensor([0.0], device=device))
                reward_dict["lifted"] = lifted
                # give a reward when the mug is at the target position
                reached_target = torch.where(distance < 0.05, torch.tensor([1.0], device=device), torch.tensor([0.0], device=device))
                reward_dict["reached_target"] = reached_target
                # define total reward
                reward = reward_dict["distance_to_target"] + 2.0 * reward_dict["lifted"] + 5.0 * reward_dict["reached_target"]
            else:
                raise NotImplementedError
        else:
            raise NotImplementedError
        ...
        self.scores = (0.151 - torch.norm(mug_pose[..., :3] - goal_mug_pose[:3], dim=-1)) / 0.151
        ...
        self.extras['indicator'] = self.mug_pose.clone().mean(dim=0)
    ...
    def compute_observations(self):
        ...
        self.mug_pose = self.rigid_body_states[:, self.mug_body_handle, 0:7]
        self.mug_linvel = self.rigid_body_states[:, self.mug_body_handle, 7:10]
        self.mug_angvel = self.rigid_body_states[:, self.mug_body_handle, 10:13]
    ...
    def _setup_attachable_body(self, ):
        ...
        for object_name in ['apple', 'fork', 'plate', 'sugar_box', 'tomato_soup_can', 'mug']:
            ## load attach info
