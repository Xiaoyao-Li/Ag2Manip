...
class PickupCube(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        if self.is_planning or self.act_type in ['robot_joint_pose']:
            ...
            robot_start_pose.p = gymapi.Vec3(0.0, -0.4, 1.2)
            robot_start_pose.r = gymapi.Quat(*R.from_euler('xyz', [0., 0., 1.57]).as_quat())
        else:
            robot_start_pose = gymapi.Transform()
            robot_start_pose.p = gymapi.Vec3(0.0, -0.2, 1.7)
        ...
        for object_asset_name in ["floor", "room_0", "room_1", 'cube', 'table']:
        ...
        if self.camera in [None]:
            ...
            self.camera_place_pos = gymapi.Vec3(0.0, -0.3, 1.3)
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.4, 1.3)
        elif self.camera == 'right':
            # self.camera_place_pos = gymapi.Vec3(0.2, -0.3, 1.3)
            # self.camera_lookat_pos = gymapi.Vec3(-0.1, 0.4, 1.3)
            self.camera_place_pos = gymapi.Vec3(0.2, -0.2, 1.3)
            self.camera_lookat_pos = gymapi.Vec3(-0.2, 0.2, 1.3)
            ...
            self.camera_place_pos = gymapi.Vec3(-1.5, -1.3, 2.0)
            self.camera_lookat_pos = gymapi.Vec3(0.3, -0.5, 1.0)
        else:
    ...
    def compute_reward(self, actions):
        ...
        if self.repre_type in ["r3m", "ag2manip", "vip", ]:
            ...
            goal_cube_pose = torch.tensor([0.0, 0.0, 1.4, 0., 0., 0., 1.], device=self.device)
            cube_pose = self.cube_pose.clone()
            achived_goal = torch.norm(cube_pose[..., :3] - goal_cube_pose[:3], dim=-1) < 0.1

            reward = (0.166 - torch.norm(cube_pose[..., :3] - goal_cube_pose[:3], dim=-1)) / 0.166
            reward = torch.where(reward < 0, torch.exp(reward) - 1, 10 * (torch.exp(2 * reward) - 1))
        
        elif self.repre_type in ['eureka']:
            if self.eureka_seed == 0:
                cube_pos = self.cube_pose[..., :3]
                goal_pos = torch.tensor([0.0, 0.0, 1.4], device=self.device)
                # Variables
                reward_scale = torch.tensor(1.0).to(cube_pos.device)
                pos_diff_temp = torch.tensor(1.0).to(cube_pos.device)
                # Compute the distance between the cube and the goal
                dist = torch.norm(cube_pos - goal_pos)
                # Compute the position difference and apply an exponential transformation with a temperature parameter 
                pos_diff = torch.exp(-pos_diff_temp * dist)
                # Scale the reward by some factor
                reward = reward_scale * pos_diff
            elif self.eureka_seed == 1:
                # locating cube position in the environment
                cube_pos = self.cube_pose[:, 0:3] 
                goal_pos = torch.tensor([0.0, 0.0, 1.4], device=self.device)
                # calculate the distance between cube and the target position
                dist_to_goal = torch.norm(cube_pos - goal_pos, dim=-1)
                # use exponential transformation on distance to goal with a temperature parameter
                temperature = torch.tensor(0.2).to(goal_pos.device)
                dist_reward = torch.exp(-temperature * dist_to_goal)
                # see if the cube is at the target height, if so, reward the agent
                target_height = goal_pos[2] 
                achieved_goal = (cube_pos[:, 2] >= target_height).float()
                achieved_reward = 10.0 * achieved_goal
                # total reward is sum of distance reward and achievement reward
                reward = dist_reward + achieved_reward
            elif self.eureka_seed == 2:
                cube_pose = self.cube_pose.clone()[:, :3]
                goal_pos = torch.tensor([0.0, 0.0, 1.4], device=self.device)
                # Initialize reward parts
                reward_dict = {"dist_reward": torch.tensor(0., device=cube_pose.device),
                            "height_reward": torch.tensor(0., device=cube_pose.device),
                            "goal_reward": torch.tensor(0., device=cube_pose.device)}
                dist_to_goal = torch.norm(cube_pose - goal_pos)
                height_of_cube = cube_pose[..., 2]  # get z-index which is height 
                goal_height = goal_pos[..., 2]  # get z-index from the goal position
                # If the cube is closer to the goal, then the reward increases
                dist_reward = -dist_to_goal
                # If the cube is higher than the initial height (1.234), then the reward increases
                height_reward = torch.zeros(self.num_envs)
                height_reward = torch.where(height_of_cube > 1.234, 
                                            torch.zeros_like(height_reward, device=cube_pose.device), 
                                            -torch.ones_like(height_reward, device=cube_pose.device))
                # If the cube is within a certain tolerance of the goal position, then additional reward obtained
                goal_reward = torch.where(torch.abs(dist_to_goal) <= 0.01, 
                                        torch.tensor(1., device=cube_pose.device), 
                                        torch.tensor(0., device=cube_pose.device))
                # update reward fragments to the reward dictionary
                reward_dict.update({"dist_reward": dist_reward, "height_reward": height_reward, "goal_reward": goal_reward})
                # total reward
                reward = dist_reward + height_reward + goal_reward
            else:
                raise NotImplementedError
        else:
            raise NotImplementedError
        ...
        self.scores = (0.166 - torch.norm(cube_pose[..., :3] - goal_cube_pose[:3], dim=-1)) / 0.166
        ...
        self.extras['indicator'] = self.cube_pose.clone().mean(dim=0)
    ...
    def _setup_attachable_body(self, ):
        ...
        for object_name in ['cube']:
            ## load attach info
            if object_name == 'cube':
                object_attachs = pickle.load(open(os.path.join(env_asset_root, 'attachable', f'attach_{object_name}2.pkl'), 'rb'))['data']
